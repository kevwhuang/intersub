...
import Event from '../../src/sections/Event.astro';
...
describe('Event', () => {
    ...
    beforeAll(async () => {
        ...
        fallbackHtml = await container.renderToString(Event, {
            props: { event: { ...event, content: SAMPLE_CONTENT, cover: undefined, level: undefined, time: '' } },
        });

        html = await container.renderToString(Event, { props: { event } });
        unresolvedHtml = await container.renderToString(Event, { props: { event: { ...event, cover: UNRESOLVED_COVER } } });
    });

    test('renders the back link to the events index', () => {
        expect(html).toContain('class="event__back" href="/events"');
        expect(html).toMatch(/<span data-i18n="All events"[^>]*>All events<\/span>/);
    });

    test('renders the event title as the headline', () => {
        expect(html).toMatch(/<h1 class="event__title" data-scroll[^>]*>/);
        expect(html).toContain(`>${event.title}</h1>`);
    });

    test('renders date, time, location, and who meta rows', () => {
        for (const label of META_LABELS) {
            expect(html).toContain(`class="event__meta-label" data-i18n="${label}"`);
        }
    ...
    test('renders an eager optimized cover image tinted by level', () => {
        expect(html).toContain(`class="event__cover" data-scroll style="background:${levelMeta.cover};color:${levelMeta.ink}"`);
        expect(html).toContain(coverName);
        expect(html).toMatch(/<img[^>]*data-image-component="true"[^>]*class="event__cover-image">/);
    ...
    test('falls back to a plain image for an unresolved cover path', () => {
        expect(unresolvedHtml).toContain(`<img class="event__cover-image" alt="" decoding="async" src="${UNRESOLVED_COVER}"`);
    ...
    test('renders aria-hidden initials only when the event has no cover', () => {
        expect(fallbackHtml).toMatch(new RegExp(`<span class="event__cover-initials" aria-hidden="true"[^>]*>${getInitials(event.title)}</span>`));
        expect(fallbackHtml.split('event__cover-image').length - 1).toBe(0);
        expect(html.split('event__cover-initials').length - 1).toBe(0);
    });

    test('transforms markdown headings, list runs, and paragraphs into content blocks', () => {
        expect(fallbackHtml).toMatch(/<div class="event__content prose" data-scroll[^>]*><p[^>]*>Intro paragraph\.<\/p><h2[^>]*>Focus<\/h2><ul[^>]*><li[^>]*>Point one<\/li><li[^>]*>Point two<\/li><\/ul><p[^>]*>Closing paragraph\.<\/p><\/div>/);
    ...
    test('links the register cta to the prefilled contact form', () => {
        expect(html).toContain(`class="button event__action" href="/?event=${encodeURIComponent(event.title)}#contact"`);
        expect(html).toMatch(/data-i18n="Register"[^>]*>Register</);
    });

    test('links back to all events from the actions row', () => {
        expect(html).toContain('class="button button--outline event__action" href="/events"');
        expect(html).toMatch(/data-i18n="Back to all"[^>]*>Back to all</);
    });

    test('marks the title, meta, cover, content, and actions for scroll animation', () => {
