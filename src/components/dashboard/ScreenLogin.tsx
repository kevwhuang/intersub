            ...
            <div style={{ maxWidth: 420, width: '100%' }}>
                ...
                <div style={{ background: STYLES.colorSurface, border: STYLES.border, borderRadius: 18, boxShadow: STYLES.shadowCard, marginBottom: 24, padding: 'clamp(28px, calc(24px + 1.25vw), 40px) clamp(24px, calc(20px + 1.25vw), 36px)' }}>
                    ...
                    >
                        ...
                        <button className="dashboard-button dashboard-button--primary" aria-label="Sign in" disabled={auth.isPending} style={{ alignItems: 'center', display: 'inline-flex', justifyContent: 'center', minHeight: 47, padding: '14px 0' }} type="submit">
                            {auth.isPending ? <Spinner /> : 'Sign in'}
                        </button>
                    </form>
                </div>
                <a className="dashboard-link" href="/" style={{ display: 'block', fontSize: 12, margin: '0 auto', width: 'fit-content' }}>&larr; Back to site</a>
            </div>
        </div>
    );
}
