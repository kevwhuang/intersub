    ...
    return (
        <header style={{ alignItems: 'center', backdropFilter: 'blur(12px)', background: STYLES.colorSurface90, borderBottom: STYLES.border, display: 'flex', flex: 'none', gap: 14, height: 60, padding: '0 16px', position: 'sticky', top: 0, zIndex: 40 }}>
            ...
            <div style={{ flex: 1, position: 'relative' }}>
                <input className="dashboard-input" aria-label="Search" value={searchValue} onChange={event => onSearchChange(event.target.value)} placeholder="Search&hellip;" style={{ background: STYLES.colorSurfaceRaised, borderRadius: STYLES.borderRadiusSmall, color: STYLES.colorInk, fontSize: 16, padding: '9px 12px 9px 14px', width: '100%' }} />
            </div>
        </header>
    );
}
