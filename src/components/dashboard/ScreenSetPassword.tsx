                ...
                <div style={{ background: STYLES.colorSurface, border: STYLES.border, borderRadius: 18, boxShadow: STYLES.shadowCard, padding: 'clamp(28px, calc(24px + 1.25vw), 40px) clamp(24px, calc(20px + 1.25vw), 36px)' }}>
                    ...
                    >
                        <label>
                            ...
                            <input className="dashboard-input" autoComplete="new-password" maxLength={PASSWORD_MAX} minLength={PASSWORD_MIN} onChange={event => setPassword(event.target.value)} placeholder="8&ndash;20 characters" style={STYLES.inputBase} type="password" value={password} />
                        </label>
                        {auth.error && <p role="alert" style={{ color: STYLES.colorError, fontSize: 12, margin: 0 }}>{auth.error}</p>}
                        <button className="dashboard-button dashboard-button--primary" aria-label="Set password" disabled={auth.isPending} style={{ alignItems: 'center', display: 'inline-flex', justifyContent: 'center', marginTop: 8, minHeight: 47, padding: '14px 0' }} type="submit">
                            {auth.isPending ? <Spinner /> : 'Set password'}
                        </button>
                    </form>
                </div>
                <button className="dashboard-link" onClick={auth.handleCancelSetPassword} style={{ background: 'none', border: 'none', display: 'block', fontSize: 12, margin: '24px auto 0', textAlign: 'center' }} type="button">
                    &larr;
                    {' '}
