            ...
            <div style={{ maxWidth: 420, width: '100%' }}>
                ...
                <div style={{ background: STYLES.colorSurface, border: STYLES.border, borderRadius: 18, boxShadow: STYLES.shadowCard, marginBottom: 24, padding: 'clamp(28px, calc(24px + 1.25vw), 40px) clamp(24px, calc(20px + 1.25vw), 36px)' }}>
                    ...
                    >
                        ...
                        <button className="dashboard-button dashboard-button--primary" aria-label="Set password" disabled={auth.isPending} style={{ alignItems: 'center', display: 'inline-flex', justifyContent: 'center', minHeight: 47, padding: '14px 0' }} type="submit">
                            {auth.isPending ? <Spinner /> : 'Set password'}
                        </button>
                    </form>
                </div>
                <button className="dashboard-link" onClick={auth.handleCancelSetPassword} style={{ background: 'none', border: 'none', display: 'block', fontSize: 12, margin: '0 auto', textAlign: 'center' }} type="button">
                    &larr;
                    {' '}
